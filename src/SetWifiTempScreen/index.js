...
class SetWifiTempScreen extends Component {
   ...
   initwifi = (ssid, password) => {
    ...
    const data1 = stringToBytes(ssid);
    const data2 = stringToBytes(password);
    console.log(dua)
    console.log(data1)
    console.log(data2)
    ...
    BleManager.retrieveServices(id).then((peripheralInfo) => {
      setTimeout(() => {
        BleManager.write(id, serviceID, '78604f25-789e-432e-b949-6fb2306fd5d7', dua).then(() => {
          ...
          setTimeout(() => {
            BleManager.write(id, serviceID,'98a8d501-07ab-42a9-94e1-d590839bf71b', data1).then(() => {
              console.log("Success Write Password");
              setTimeout(() => {
        BleManager.write(id, serviceID, '2b9310ca-d12c-4940-a436-c33e32be84c7', data2).then(() => {
